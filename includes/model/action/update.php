...
<?php
    ...
    add_action('wp_ajax_UpdateThisAgent', 'UpdateThisAgent');
    add_action('wp_ajax_nopriv_UpdateThisAgent', 'UpdateThisAgent');
    function UpdateThisAgent() 
    { 
        if( !isset($_POST['cluster_id']) || !isset($_POST['cluster_name'])  || !isset($_POST['cluster_info']) || !isset($_POST['cluster_hostname']) || !isset($_POST['cluster_capacity']) || !isset($_POST['mcafee_ready']) )
        {
            echo json_encode( 
                array(
                    'status'=>'danger', 
        ...
        $mcafee_ready = $_POST['mcafee_ready'];

        global $wpdb; //Reference to wp mysql conn.
        $clusterTable = SP_SECRET_TAB;
        ...
        $updates = $wpdb->get_results( "UPDATE $clusterTable SET cluster_name = '$cluster_name', cluster_info = '$cluster_info', cluster_hostname = '$cluster_hostname', cluster_capacity = '$cluster_capacity', cluster_capacity = '$cluster_capacity', mcafee_ready = '$mcafee_ready' WHERE ID = '$cluster_id'" );
